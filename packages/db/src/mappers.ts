...
export function mapMailbox(row: typeof mailboxesTable.$inferSelect): MailboxSettings {
  return mailboxSettingsSchema.parse({
    ...
    gmailMailboxAddress: row.gmailMailboxAddress ?? null,
    gmailLabelFilter: row.gmailLabelFilter ?? null,
    allowedSenderPatterns: parseJson(row.allowedSenderPatternsJson),
    allowedOutboundRecipientPatterns: parseJson(row.allowedOutboundRecipientPatternsJson),
    enableLiveRead: row.enableLiveRead,
    enableLiveDrafts: row.enableLiveDrafts,
    enableLiveSend: row.enableLiveSend,
...
export function mapThread(row: typeof threadsTable.$inferSelect): EmailThread {
  return emailThreadSchema.parse({
    ...
    latestExternalHistoryId: row.latestExternalHistoryId ?? null,
...
export function mapMessage(row: typeof messagesTable.$inferSelect): EmailMessage {
  return emailMessageSchema.parse({
    ...
    externalThreadId: row.externalThreadId ?? null,
    externalHistoryId: row.externalHistoryId ?? null,
...
export function mapDraft(row: typeof draftsTable.$inferSelect): DraftReply {
  return draftReplySchema.parse({
    ...
    providerName: row.providerName,
    externalDraftId: row.externalDraftId ?? null,
    externalMessageId: row.externalMessageId ?? null,
